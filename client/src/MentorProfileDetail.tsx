...
export default function MentorProfilePage() {
  ...
  // URL Query String (?id=1) se ID Get karna
  const mentorId = useMemo(() => {
    const params = new URLSearchParams(window.location.search);
    return params.get('id');
  }, []);

  // Selected Mentor search karna (Fallback to first mentor if not found)
  const mentor: Mentor = useMemo(() => {
    if (!mentorId) return INITIAL_MENTORS[0];
    const found = INITIAL_MENTORS.find((m) => m.id === Number(mentorId));
          ...
          <div className="lg:col-span-6 space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-1.5 flex items-center gap-1 overflow-x-auto scrollbar-none">
              {[
                ...
                { name: 'Mock Tests', icon: FileText },
                { name: 'Assignments', icon: FileText },
                { name: 'Study Materials', icon: BookOpen },
                // { name: `Reviews (${mentor.reviewsCount})`, icon: Star },
