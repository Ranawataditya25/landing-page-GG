...
const MentorCard: React.FC<MentorCardProps> = ({ name, role, experience, description, imageUrl, onBookSession }) => (
  <div className="flex flex-col justify-between rounded-2xl border border-slate-100 bg-white p-4 shadow-sm hover:shadow-md transition-shadow">
    <div>
      <div className="flex items-center gap-3">
        <img
    ...
    <button
      ...
      className="mt-4 flex w-full items-center justify-center gap-1.5 rounded-lg bg-indigo-50 py-1.5 text-xs font-semibold text-indigo-600 transition-colors hover:bg-indigo-100 cursor-pointer"
    >
      <Calendar className="h-3.5 w-3.5" />
      Book a Session
    </button>
...
export default function PsychologicalSupport() {
  const handleNavigateToPhyclass = () => {
    if (typeof window !== 'undefined') {
      window.location.href = '/phyclass';
    }
  };
      ...
      <div className="w-full max-w-6xl space-y-6">
        ...
        <section className="rounded-3xl border border-indigo-50/80 bg-[#FAF9FD] p-6 md:p-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {mentors.map((mentor, index) => (
              <MentorCard 
                key={index} 
                {...mentor} 
                onBookSession={handleNavigateToPhyclass}
              />
            ))}
          </div>
          ...
          <div className="mt-6 flex flex-col items-center justify-between gap-4 rounded-2xl border border-indigo-100 bg-white p-4 shadow-sm md:flex-row">
            ...
            <button
              type="button"
              onClick={handleNavigateToPhyclass}
              className="flex shrink-0 items-center gap-2 rounded-xl bg-[#4F35D2] px-5 py-2.5 text-xs font-semibold text-white shadow-sm transition hover:bg-indigo-700 active:scale-95 cursor-pointer"
            >
              <MessageCircle className="h-4 w-4" />
              Talk to a Mentor
            </button>
